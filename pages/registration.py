...
with st.form(key="paalna_form", clear_on_submit=False):
    ...
    with col2:
        ...
        location = st.text_input("8. Location*", placeholder="e.g., School Garden")
...
if submit_button:
    if not all([student_name, student_class, school_name, tree_name, species, location, teacher_name, holiday_guardian, photo]):
        ...
        try:
            ...
            # Query returning inserted sequential ID
            insert_query = text("""
            ...
            cert_pdf_bytes = generate_paalna_certificate(
            ...
            # Use download button directly for PDF files (st.image cannot display raw PDF bytes)
            st.download_button(
                label="📥 Download Certificate (PDF)",
                data=cert_pdf_bytes,
                file_name=f"PAALNA_Certificate_{student_name.replace(' ', '_')}.pdf",
